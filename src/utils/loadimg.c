...
ImageData LoadBMP(const char *path)
{
    ...
    int16_t bpp = (*(uint16_t *)&header[COLOR_SIZE_OFFSET]) / 8;
    uint8_t padding = (PADDING_BASE - bpp);
    uint8_t eofPadding = imgDat.w % 4;

    imgDat.dat = malloc(imgDat.w * imgDat.h * 4);

    fseek(fptr, *(uint32_t *)&header[PIXEL_ARRAY_ADDRESS], SEEK_SET); // go to start of pixel array
